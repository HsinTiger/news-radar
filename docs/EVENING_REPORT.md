...
## 📦 morning 新增的 6 個 commit（在 7f393bd..HEAD 之間）
...
4. **config.yaml 裡的老舊 key**（`schedule.publishing_slots` / `schedule.jitter`）— Phase 8.15 後已不讀這些 key，但還留在 config 裡——清乾淨會讓新人不會誤以為還有這機制
