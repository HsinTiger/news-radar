...
Substack drafts carry 🖼 markers in the body (format defined by
`substack_radar/composer.py:706-711`). Each marker holds a scene description, a
Path B search string, and a Path C generation prompt. Until 2026-08-01 those
markers were always left for manual generation, because
`src/image_brain.py::generate_image` was a hard `return None` stub.

Generation now runs off a local CLI on the Mac — `codex` (`$imagegen`,
gpt-image-2) or `agy` (Antigravity image tool). No new API key: both use an
existing local login. Cloud runners have neither CLI, so
`image_engines.resolve_engine()` returns `None` there and the markers survive
untouched, matching the `llm_brain._agy_available()` fallback pattern.

**Covers do not go through this path.** Covers stay on the deterministic
Pillow route (`render_substack_cover` → `character_cover` → `promise_cover`).

**Data charts do not go through this path either.** Diffusion models fabricate
axis values and garble CJK text, which contradicts the composer's own
no-invented-numbers rule (`substack_radar/composer.py:687-693`). Only
scene and concept illustrations are generated; anything carrying real figures
needs deterministic rendering.

### Enabling it

1. On the Mac, pin the real CLI behavior — flags shift between CLI versions, so
   the defaults in `src/image_engines.py` are a starting point, not a promise:

   ```bash
   python3 scripts/probe_image_engines.py
   ```

   The probe writes only to `/tmp`, and exits 1 when no engine produced a
   verifiable image. Open the produced PNG before trusting the result.

2. Paste the settings it recommends into `.env`:

   ```
   SUBSTACK_IMAGE_ENGINE=codex     # off (default) | codex | agy
   SUBSTACK_IMAGE_MAX_PER_RUN=3
   ```

`codex` is the default recommendation because `agy` draws on the same Google AI
Pro quota as the article composer (`src/llm_brain.py:341-347`), where each
request already spends 23-25k tokens on its own system prompt. Writing the
article outranks illustrating it; `codex` bills a separate ChatGPT pool.

### What you get back

On success the marker block is replaced by the image, its caption, and an HTML
comment holding the exact prompt that produced it (Substack does not render the
comment; it uses the same convention as the Subscribe placeholder in
`build_footer_block`). A machine-readable `inline_images.json` lands next to
the draft with prompt, filename, and engine per image.

Failure is never fatal. A missing CLI, a timeout, an exhausted quota, or an
engine that reports success without writing a real file all leave the marker
block byte-identical so it can still be generated by hand. Success is
determined by `image_engines.verify_image` — file exists, non-zero, opens as an
image — never by the subprocess exit code, which CLI agents routinely return as
0 after writing nothing.
