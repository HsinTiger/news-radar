...
- `PROVEN`: D1 readback at 2026-07-24 23:05 Asia/Taipei reports `AUTOMATION_MODE=recovery`, `SUBMISSION_PROCESSOR_MODE=paused`, and zero current-v10 production evaluations for all three platforms. The two older Threads Recovery rows still say carousel and are legacy evidence. The next truth gate is the real 2026-07-25 08:07 Asia/Taipei Threads slot on main `870a7e6`: require feed format, a non-empty Meta post ID, API readback, canonical persistence, D1 experiment visibility, and later 1h/24h/168h snapshots.
