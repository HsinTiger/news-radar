...
- `PROVEN`: PR #56 raised numeric grounding and the current guard is `2026-07-24.taiwan-daily-v9`. Material Arabic-number claims in captions or rendered carousel cards must exist in the supplied title, article, same-event context, or source timestamp; a fabricated or model-added date/amount becomes `unsupported_numeric_claim` and cannot enter the Recovery queue. The bounded rewrite receives the exact source numeric allowlist plus an explicit reader-utility shape.
