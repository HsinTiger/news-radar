...
- `PROVEN`: PR #56 raised numeric grounding. That rule remains active in the current guard, `2026-07-24.taiwan-daily-v10`: material Arabic-number claims in captions or rendered carousel cards must exist in the supplied title, article, same-event context, or source timestamp; a fabricated or model-added date/amount becomes `unsupported_numeric_claim` and cannot enter the Recovery queue.
...
- `PROVEN`: PR #62 makes the Recovery format experiment truthful. Facebook and Threads publish native feed text plus a single image; only Instagram publishes the five-card carousel. Compose and pre-publish checks inspect only content that platform will expose, planned format is stored per platform, and a failed Instagram carousel remains queued instead of silently degrading to a single image. Local regression passed `628` tests. Setup-only run `30103198346` executed v10 and correctly held all candidates for source/editorial defects; it did not touch Meta or canonical state.
- `PROVEN`: PR #63 compiles each Recovery composer request from the source evidence: a normalized numeric allowlist, one required non-null platform, null unrequested platforms, and an Instagram-only five-card schema without legacy example numbers. Setup-only run `30104018447` passed v10 after the high-speed-rail candidate fixed reader utility in one rewrite; the first food-safety candidate remained held for source-external `25`. The disposable queue changed `0` to `1`, while canonical state and `publish_log=809` remained unchanged. Full regression passed `630` tests.
- `PROVEN`: D1 readback at 2026-07-24 23:05 Asia/Taipei reports `AUTOMATION_MODE=recovery`, `SUBMISSION_PROCESSOR_MODE=paused`, and zero current-v10 production evaluations for all three platforms. The two older Threads Recovery rows still say carousel and are legacy evidence.
- `PROVEN`: PR #65 adds the missing remote proof gate. After the local publish-log verifier, Full Pipeline sends every just-created requested platform post ID back through the corresponding Meta insights API. Readable zero metrics pass; a missing ID, token/API error, or unreadable post fails. State persistence remains unconditional, so a remote-readback red result cannot authorize duplicate publishing. Full regression passed `633` tests and workflow YAML parsing passed.
- `PROVEN`: main-head Threads setup-only run `30104811736` passed v10 on runtime commit `97955c8`: the first food-safety candidate remained held for source-external `25`; the high-speed-rail candidate passed after one reader-utility rewrite; the disposable queue changed `0` to `1`, while canonical state and `publish_log=809` remained unchanged. The next truth gate is the real 2026-07-25 08:07 Asia/Taipei Threads slot on main containing runtime commit `97955c8`: require feed format, non-empty Meta post ID, new remote-readback PASS, canonical persistence, D1 experiment visibility, and later 1h/24h/168h snapshots.
