...
def _build_user_prompt(
    *,
    ...
    return (
        ...
        f"=== 主題分類 ===\n{topic_category}\n\n"
        f"=== 多樣性提醒 ===\n{avoid}\n\n"
        # === 2026-06-01 風格硬規範（Hsin 強烈回饋：比喻太多、廢話太多、要真實分析與人類判斷）===
        "=== 動筆前先做這步（深度分析，不是修辭）===\n"
        "在心裡先回答（不要寫進文章）：這則素材的**核心張力／反直覺點**是什麼？它的**第二層含義**"
        "是什麼（影響誰、改變哪個賽局、推到極端會怎樣）？用真實機制與具體數字去解釋，不是用比喻去裝飾。\n"
        "**風格硬規範（違反任一條都算失敗）：**\n"
        ...
        "  2. **前置一句反直覺 thesis**（你的判斷）放在開場前三段內，像「X 根本不是 A，而是偽裝成 A 的 B」這種強斷言。\n"
        "  3. **每個 ▉ 小節用『具名資訊型小標』**：▉ 後接一句能單獨讀懂、帶鉤子的短句（如「平台不想要下一個 X」），不要用「第二層含義」這種抽象標籤。每個小節 = 一個讓讀者覺得『有在前進』的微目標。\n"
        ...
        "{\n"
        '  "title": "...",                  // 8-60 字。**外層放好奇/情緒鉤**（像「也許我們不會再有下一個 X」「為什麼最完美的模型最先崩潰」），**不要把硬數據/術語塞進標題**（數字與型號留給副標與內文）。可用 §6 公式(鉤子+：+payoff 或開放問句)，含一個具體錨點，禁新聞稿陳述式/震驚體/listicle。\n'
