...
def load_manny_frameworks(brief_name: str) -> str:
    ...
    return (
        ...
        "絕不可當成本期標的的事實寫進文章。\n\n"
        # 折衷點：框架堆多了會讓寫手為了滿足檢查表而防守，每句合格、整篇沒人味。
        # 所以明確區分硬規則與手藝建議，並告訴寫手可數的東西有機器在管，
        # 不必在寫作當下分心盯格式。
        "**規則的份量**：以下只有三條是硬規則，違反就是錯——"
        "① 不虛構，沒有的證據寫「查不到」；"
        "② 每個判斷都要能被推翻；"
        "③ 框架的舉例不是本期事實。\n"
        "其餘全部是手藝建議，描述「通常這樣比較好」，不是「不准那樣」。"
        "有理由違反、而且文章因此更好，就違反它——刻意的破格是寫作的一部分。"
        "衝突時：**文章好看 > 規則整齊**。\n"
        "名詞化、比較基準、子標題冒號、對仗句這幾項有機器事後檢查，"
        "**先把文章寫好，格式問題交給機器**，不要為了閃避規則而寫得綁手綁腳。\n\n"
        "**怎麼用這些框架（先讀這段）**\n"
        "以下規則分兩種，份量不同：\n\n"
        "**硬規則（只有三條，違反就是錯）**\n"
