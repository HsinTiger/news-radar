...
def test_v17_rejects_actual_recovery_canary_editorial_shapes() -> None:
    threads = (
...
def test_v17_accepts_natural_threads_shape_without_editorial_labels() -> None:
    text = (
...
def test_v17_threads_rejects_oversized_copy_and_impersonal_question() -> None:
    text = (
...
def test_v17_threads_rejects_stat_dump_and_vague_stock_question() -> None:
    text = (
...
def test_v17_dates_and_rule_numbers_do_not_count_as_stat_overload() -> None:
    text = (
...
def test_v17_formal_reader_pronoun_is_a_direct_question() -> None:
    text = (
        "證交所公告隆銘綠能恢復一般交易方式。\n\n"
        "股東可在券商平臺確認交易方式。\n\n"
        "您會調整隆銘綠能持股比例嗎？\n\n#台股"
