...
def test_v18_rejects_actual_recovery_canary_editorial_shapes() -> None:
    threads = (
...
def test_v18_accepts_natural_threads_shape_without_editorial_labels() -> None:
    text = (
...
def test_v18_threads_rejects_oversized_copy_and_impersonal_question() -> None:
    text = (
...
def test_v18_threads_rejects_stat_dump_and_vague_stock_question() -> None:
    text = (
...
def test_v18_dates_and_rule_numbers_do_not_count_as_stat_overload() -> None:
    text = (
...
def test_v18_formal_reader_pronoun_is_a_direct_question() -> None:
    text = (
        "證交所公告隆銘綠能恢復一般交易方式。\n\n"
        "股東可在券商平臺確認交易方式。\n\n"
        "您會調整隆銘綠能持股比例嗎？\n\n#台股"
...
def test_v18_threads_rejects_two_closing_questions() -> None:
    text = (
        "證交所公告本週加權指數上漲2.3%。\n\n"
        "投資人可對照自己的同期間報酬。\n\n"
