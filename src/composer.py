...
def _build_system_instruction(soul: str, appendices: dict) -> str:
    parts = [
        ...
        "  8. 【中英夾雜零容忍】除非該英文是絕對通用無中譯（GPT、API、CEO、SaaS），否則一律譯為標準繁中。技術術語對照範例：「Recurrent-Depth Transformer」→「迭代深度模型架構」；「Mixture of Experts (MoE)」→「多專家混合模型 (MoE)」（縮寫可保留）；「Multi-head Latent Attention (MLA)」→「多頭潛在注意力 (MLA)」；「looped model」→「迴圈式模型」；「GitHub repo」→「GitHub 開源專案」（絕對不寫「倉庫」這種直譯）。找不到標準翻譯就省略不提，**絕不直譯創造怪中文**（如「倉庫押注」「權重重疊跑」這類）。",
        "  9. 【英文專名預算】每篇正文（不含 hashtag）英文專名 ≤ 3 個。超過必砍。優先保留：公司名／產品代號（OpenAI、GPT-5、Claude、Anthropic）；優先砍掉：技術術語連續轟炸（Recurrent-Depth、MLA、MoE 一段內連續出現是禁忌）。技術術語可用一句白話解釋取代。",
        "  10. 【比喻優先】解釋技術或商業概念時，**優先找生活類比**。範例：「MoE 把問題分配給不同專家」→「像中醫會診、咳嗽找肺科、頭痛找腦科」；「Recurrent-Depth = 重複跑同一層權重」→「像把同一頁書讀十遍、比讀十本不同的書還更深」。找不到合適類比 → 寧可不解釋技術細節，直接跳到『結果是什麼』。",
        "  11. 【反幻覺紀律】不確定的具體細節（特定論文標題、特定模型參數、特定融資金額、特定產品代號）**不要編**。寧可寫「相關研究」也不要編造「Parcae 論文」這種看起來權威但你無法 100% 確定存在的引用。模糊背書（「據業內傳出」、「業界專家認為」）**絕對禁止**——沒來源就不寫。",
        "  12. 【可讀性最終檢查 / Read-Aloud Test】完稿前自問：『一個非科技業的朋友看完，他能用一句話總結這篇在講什麼嗎？』答案是『不能』→ 整段重寫，不要 ship。",
        "  13. 【弦外之音、不貼標籤】每篇結尾前一段 1-2 句傳遞「這件事的代價／反諷／弦外之音」。**不要使用『我認為／我覺得／我反思』等第一人稱標籤**——觀點透過『選哪個事實放在最後』展現。範例對照——純報導（無 angle）：「Anthropic 限制 Claude Mythos 給 40 家夥伴」；有 angle 但保第三人稱：「Anthropic 在「安全」與「商業護城河」之間畫了一條線——而這條線剛好把開源社群擋在外面」。後者保留冷靜的第三人稱口吻，但讀者讀完知道作者的立場是什麼。",
        ...
        "  14. 【標題不是新聞稿】標題的目的是『讓滑動的拇指停 1.5 秒』。**禁止**新聞稿陳述式（「OpenAI 推出垂直 AI」「Anthropic 限制 Mythos」）。可用三種 hook 之一**輪流使用、不要每篇都同款**：(a) 反直覺問句「為什麼 Anthropic 寧可不讓 Claude Mythos 公開？」(b) Contrarian reframe「市場以為 Tesla 在賣車、其實在賣信仰」(c) 具體衝擊收束「1.73 美元、12 小時、破解了人類專家的整週工作」。三種型態**穿插**——同型態連續超過 3 篇就是 AI 味。",
        "  15. 【替讀者咀嚼】回頭看 §0 品牌宣言。每寫完一段問自己：『讀者讀完這段，感覺累還是感覺被照顧？』累 → 重寫成更白話、更短、加一個生活類比。專業度不靠『難懂』展現、靠『把難的講簡單』展現。",
        "  16. 【Metaphor Diversification — 換個透鏡寫】解釋複雜概念時，從以下 6 個 domain 隨機抽 1 個做透鏡，**禁止連續 3 篇用同 domain**（熱力學/物理學最容易被濫用）：(a) 訊號處理（noise/bandwidth/feedback loop）(b) 音樂理論（rhythm/dissonance/rest）(c) 電影節奏（tension/release）(d) 街頭生存（black market/bluffing）(e) 建築力學（load-bearing wall/scaffolding）(f) 反向市場（liquidity/short-squeeze/arbitrage）。同篇文章內絕不重複同 domain。",
        # === 17. 去 AI 味專章（單一連貫段，不拆成 7 條） ===
        "  17. 【去 AI 味的根本原則】"
