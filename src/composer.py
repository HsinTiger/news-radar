...
def _build_system_instruction(soul: str, appendices: dict) -> str:
    parts = [
        ...
        # === 2026-06-02: Hook Diversity Enforcement — 解決「太扯了」模板病 ===
        # 每次產出三個版本的開頭必須不同類型。禁止連續使用同一種開場句結構。
        # 檢查過去的貼文（如果有）確保這次的開頭跟最近 3 篇不同。
        "🔥【開場句多樣性強制規則——高於所有其他規則】🔥",
        "  Threads 開場句：從以下類別輪流使用，**禁止連續 2 篇用同類型**：",
        "    ①「這讓我想到⋯⋯」類比連結型",
        "    ②「大家在看 X，我在看 Y」降維打擊型",
        "    ③「我原本以為 X，結果⋯⋯」自我修正型",
        "    ④「更反直覺的是⋯⋯」翻轉預期型",
        "    ⑤「大家都在討論 X，真正要注意的是 Y」逆向型",
        "    ⑥「以前還覺得⋯現在才發現⋯」遞進型",
        "    ⑦「太扯了⋯⋯」情緒衝擊型（**限定：每 10 篇最多 1 次**）",
        "  如果你不確定上次用了哪種，就用 ①（因為 ① 最自然、最少人用）。",
        "",
        "  FB開場句：從以下類別輪流使用，**禁止連續 2 篇同類型**：",
        "    A. 事實錨點開場（「X 最新財報顯示⋯⋯」）",
        "    B. 類比開場（「這讓我想到 2008 年⋯⋯」）",
        "    C. 反直覺開場（「大家都在看 Y，其實真正該注意的是 Z」）",
        "    D. 趨勢總結開場（「X 正在發生一個結構性的變化⋯⋯」）",
        "",
        "  Threads 結尾互動語也多樣化：輪流用「你怎麼看？」「有人也在盯這件事嗎？」「這是我目前的假設」「下週法說會我會盯這個」",
        "  禁止連續 3 篇用同一種結尾。",
        "",
