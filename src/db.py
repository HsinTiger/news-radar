...
def _migrate_log_scale_engagement(conn: sqlite3.Connection) -> None:
    ...
    Adds:
      ...
      - engagement_stats.engaged_users / clicks for current Facebook metrics
    ...
    _migrate_add_column_if_missing(
        conn, "engagement_stats", "engaged_users", "INTEGER DEFAULT 0"
    )
    _migrate_add_column_if_missing(
    ...
    conn.execute(
        ...
        INSERT INTO engagement_stats
          (draft_id, platform, platform_post_id, fetched_at,
           ...
           views, reach, engaged_users, clicks, raw_json, post_age_bucket)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (
            draft_id,
